  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.createTable(new Table({
      ...
      columns: [
        ...
        { name: 'supplier_code', type: 'varchar', length: '50', isUnique: true },
        { name: 'name', type: 'varchar', length: '200' },
        { name: 'contact_person', type: 'varchar', length: '200', isNullable: true },
        ...
        { name: 'bank_name', type: 'varchar', length: '50', isNullable: true },
        { name: 'bank_account_number', type: 'varchar', length: '50', isNullable: true },
        { name: 'bank_account_name', type: 'varchar', length: '200', isNullable: true },
        { name: 'payment_terms_days', type: 'int', default: 30 },
    ...
    if (!suppliersExists) {
      await queryRunner.createIndex('suppliers', new TableIndex({ name: 'IDX_SUPPLIERS_COMPANY_ID', columnNames: ['company_id'] }));
      await queryRunner.createIndex('suppliers', new TableIndex({ name: 'IDX_SUPPLIERS_CODE', columnNames: ['supplier_code'], isUnique: true }));
      await queryRunner.createForeignKey('suppliers', new TableForeignKey({ columnNames: ['company_id'], referencedColumnNames: ['id'], referencedTableName: 'companies', onDelete: 'CASCADE' }));
    }
  }

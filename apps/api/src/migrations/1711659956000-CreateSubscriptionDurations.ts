...
export class CreateSubscriptionDurations1711659956000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    // Create subscription_durations table if not exists
    await queryRunner.createTable(
      new Table({
        ...
        columns: [
          ...
          { name: 'duration_months', type: 'int', isNullable: false, comment: 'Duration in months: 1, 3, 6, 12' },
          { name: 'discount_percentage', type: 'decimal', precision: 5, scale: 2, default: 0, comment: 'Discount percentage: 0, 5, 10, 20' },
          { name: 'final_price', type: 'decimal', precision: 15, scale: 2, isNullable: false, comment: 'Final price after discount' },
    ...
    const hasFk = table?.foreignKeys.some((fk) => fk.columnNames.includes('plan_id'));
    if (!hasFk) {
      await queryRunner.createForeignKey('subscription_durations', new TableForeignKey({
        columnNames: ['plan_id'],
    ...
    const hasIndex = table?.indices.some((i) => i.name === 'idx_unique_plan_duration');
    if (!hasIndex) {
      await queryRunner.createIndex('subscription_durations', new TableIndex({
    ...
    // Seed default durations only if table is empty
    const count = await queryRunner.query(`SELECT COUNT(*) as cnt FROM subscription_durations`);

...
export class AdminDashboardController {
  ...
  async getStats() {
    const [realTotal, realActive, realSuspended, realPending] = await Promise.all([
      this.companyRepo.createQueryBuilder('c').getCount(),
      this.companyRepo.createQueryBuilder('c').where('c.status = :status', { status: 'active' }).getCount(),
      this.companyRepo.createQueryBuilder('c').where('c.status = :status', { status: 'suspended' }).getCount(),
      this.companyRepo.createQueryBuilder('c').where('c.status = :status', { status: 'pending' }).getCount(),
    ]);
  ...
  async getMemberGrowth(@Query('period') period: string = 'month') {
    const now = new Date();
  ...
  async getTopPlans(@Query('limit') limit: number = 5) {
